...
When the user transliterated an English technical term into Bengali script, the canonical form is the Latin-script version. Flag this in the SUGGESTIONS section, not the CHANGES section. The user might have a reason (e.g. the post is targeted at non-English-readers, or the term is being introduced).
...
| সার্ভার | server | code-mix: consider "server" (also valid Bengali: সার্ভার is sometimes accepted, but server is dominant in tech) |
...
3. **If Bengali:**
   ...
   b. Check if it is a transliteration of a dominant English tech term. If yes, flag in SUGGESTIONS (not CHANGES).
